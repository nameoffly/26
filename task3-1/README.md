...
- **周级别记录**: 2777行
- **年龄范围**: 14-82岁，平均38.8岁
- **行业分布**（5大类）: Acting(33.5%), Sports(23.5%), Media(18.3%), Music(14.7%), Other(10.0%)

### 行业分类映射

原始数据包含26种行业，为提高统计稳定性，合并为5大类：

| 行业大类 | 包含的原始行业 |
|----------|----------------|
| Acting | Actor/Actress, Comedian, Magician |
| Sports | Athlete, Racing Driver |
| Music | Singer/Rapper, Musician |
| Media | TV Personality, Radio Personality, News Anchor, Sports Broadcaster, Journalist |
| Other | Model, Social Media Personality, Businessperson, Politician, Reality TV Star, Royalty, Chef, Scientist, Conservationist, Astronaut, Author, Fashion Designer, Activist 等 |
...
| ANOVA | 各行业粉丝投票相同 | 至少两个行业不同 |
| ANOVA | 各行业评委评分相同 | 至少两个行业不同 |
| t检验 | 美国vs非美国粉丝投票相同 | 粉丝投票存在差异 |
| t检验 | 美国vs非美国评委评分相同 | 评委评分存在差异 |
...
创建因素影响横向对比表，比较各因素对不同因变量的影响是否一致：

| 因素 | 因变量 | 检验方法 |
|------|--------|----------|
| 年龄 | 名次/粉丝投票/评委评分 | Pearson相关 |
| 行业 | 名次/粉丝投票/评委评分 | ANOVA |
| 美国vs非美国 | 获奖/粉丝投票/评委评分 | 卡方检验/t检验 |
...
#### 7. `factor_comparison.csv` - 因素影响横向对比表

| 列名 | 说明 |
|------|------|
| 因素 | 自变量名称（年龄、行业、美国vs非美国） |
| 因变量 | 被影响的变量（最终名次、粉丝投票、评委评分、是否获奖） |
| 检验方法 | 使用的统计检验（Pearson、ANOVA、卡方检验、t检验） |
| 统计量 | 检验统计量的值（r、F、χ²、t） |
| p值 | 显著性水平 |
| 效应方向 | 影响的方向（如"年龄↑名次↑"表示年龄越大名次越差） |
| 显著性 | 显著性标记（***p<0.001, **p<0.01, *p<0.05, ns=不显著） |

**如何解读**：
- 横向比较同一因素对不同因变量的影响是否一致
- 纵向比较不同因素对同一因变量的影响强度
- 重点关注"显著性"列，***表示高度显著
...
#### 9. `factor_comparison.png` - 因素影响横向对比图

**内容**：
- 左图：显著性热力图，展示各因素对不同因变量的影响显著性
- 右图：年龄效应对比条形图，展示年龄对不同因变量的相关系数

**如何解读**：
- 左图：颜色越深（绿色）表示越显著，可快速识别哪些因素-因变量组合有显著关系
- 右图：条形高度表示相关系数绝对值，正值表示正相关，负值表示负相关
- 绿色条形表示显著，灰色表示不显著

#### 10. `regression_coefficients.png` - 回归系数森林图

**内容**：三个模型（名次、评委评分、粉丝投票）的回归系数及95%置信区间

**如何解读**：
- 每个条形代表一个变量的回归系数
- 误差线表示95%置信区间
- 绿色表示显著（p<0.05），灰色表示不显著
- 红色虚线表示零线，系数不跨越零线则显著
- 标题中的R²表示模型解释力

#### 11. `regression_age_comparison.png` - 年龄系数模型对比图

**内容**：年龄系数在三个模型中的对比

**如何解读**：
- 条形高度表示年龄系数的大小
- 正值表示年龄越大，因变量越大（如名次越差）
- 负值表示年龄越大，因变量越小（如评分越低）
- 可直观比较年龄对不同因变量的影响强度

#### 12. `regression_prediction.png` - 回归预测效果图
...
| 行业大类 | 人数 | 平均名次 | 获奖率 | 平均年龄 |
|----------|------|----------|--------|----------|
| Sports | 99 | 6.21 | 29.3% | 35.1岁 |
| Music | 62 | 6.81 | 27.4% | 38.1岁 |
| Media | 77 | 6.88 | 22.1% | 37.5岁 |
| Acting | 141 | 6.96 | 21.3% | 43.0岁 |
| Other | 42 | 7.64 | 21.4% | 36.6岁 |

**假设检验结果**：

| 检验 | 因变量 | 统计量 | p值 | 结论 |
|------|--------|--------|-----|------|
| 卡方检验 | 是否获奖 | χ²=2.77 | 0.597 | 不显著 |
| ANOVA | 最终名次 | F=1.21 | 0.308 | 不显著 |
| ANOVA | 粉丝投票 | F=0.81 | 0.518 | 不显著 |
| ANOVA | 评委评分 | F=1.17 | 0.325 | 不显著 |

**关键发现**：
- 将26种行业合并为5大类后，行业对所有因变量均无显著影响
- Sports类选手表现最好（平均名次6.21，获奖率29.3%），但差异不显著
- Acting类选手平均年龄最大（43岁），这可能是其表现稍差的原因（年龄效应混淆）
- **结论**：行业本身对比赛结果没有显著影响，评委和粉丝都不存在行业偏见
...
| 因素 | 因变量 | 检验方法 | 统计量 | p值 | 显著性 |
|------|--------|----------|--------|-----|--------|
| 年龄 | 最终名次 | Pearson | r=0.43 | <0.001 | *** |
| 年龄 | 粉丝投票 | Pearson | r=-0.34 | <0.001 | *** |
| 年龄 | 评委评分 | Pearson | r=-0.43 | <0.001 | *** |
| 行业（5大类） | 最终名次 | ANOVA | F=1.91 | 0.108 | ns |
| 行业（5大类） | 粉丝投票 | ANOVA | F=0.81 | 0.518 | ns |
| 行业（5大类） | 评委评分 | ANOVA | F=1.17 | 0.325 | ns |
| 美国vs非美国 | 是否获奖 | 卡方 | χ²=0.00 | 1.000 | ns |
| 美国vs非美国 | 粉丝投票 | t检验 | t=-0.22 | 0.824 | ns |
| 美国vs非美国 | 评委评分 | t检验 | t=-0.52 | 0.605 | ns |

**关键发现**：

1. **年龄影响**：
   - 年龄对评委评分的影响（|r|=0.43）**大于**对粉丝投票的影响（|r|=0.34）
   - **结论**：评委比粉丝更"歧视"年龄大的选手

2. **行业影响**（5大类）：
   - 行业对评委评分、粉丝投票、最终名次均无显著影响
   - **结论**：评委和粉丝都不存在行业偏见

3. **国家影响**：
   - 美国vs非美国对评委评分和粉丝投票都没有显著影响
   - **结论**：评委和粉丝都不存在国籍偏见

**总体结论**：评委和粉丝对年龄的偏好方向一致（都偏好年轻选手），但评委对年龄更敏感。行业和国籍对两者都没有显著影响。

### 5. 国家影响：美国 vs 非美国（不显著）

| 检验 | 因变量 | 统计量 | p值 | 结论 |
|------|--------|--------|-----|------|
| 卡方检验 | 是否获奖 | χ²=0.00 | 1.000 | 不显著 |
| t检验 | 粉丝投票 | t=-0.22 | 0.824 | 不显著 |
| t检验 | 评委评分 | t=-0.52 | 0.605 | 不显著 |
| 卡方检验 | 美国各州 | χ²=14.22 | 0.819 | 不显著 |

**美国 vs 非美国统计**：

| 类别 | 人数 | 获奖率 | 平均名次 | 平均粉丝投票 | 平均评委评分 |
|------|------|--------|----------|--------------|--------------|
| US | 365 | 24.1% | 6.84 | 9.79% | 10.82% |
| Non-US | 56 | 25.0% | 6.68 | 10.02% | 11.10% |
...
2. **行业影响不显著**：将26种行业合并为5大类后，行业对所有因变量均无显著影响
3. **国家/地区影响不显著**：来自不同国家或美国不同州的选手没有系统性差异
4. **舞伴选择很重要**：好的舞伴可以显著提升比赛成绩
5. **评委和粉丝偏好基本一致**：
   - **相同点**：两者都倾向于给年轻选手更高的分数，都不存在行业和国籍偏见
   - **不同点**：评委对年龄更敏感（|r|=0.43 vs 0.34）

**回答问题："Do they impact judges scores and fan votes in the same way?"**

| 因素 | 对评委评分 | 对粉丝投票 | 影响是否一致？ |
|------|-----------|-----------|---------------|
| 年龄 | 显著负相关 (r=-0.43) | 显著负相关 (r=-0.34) | **方向一致，但评委更敏感** |
| 行业（5大类） | 不显著 (p=0.325) | 不显著 (p=0.518) | 一致（都无影响） |
| 美国vs非美国 | 不显著 | 不显著 | 一致（都无影响） |
